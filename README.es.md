...
[![Pruebas](https://img.shields.io/badge/pruebas-223-8E8E93?style=flat-square)](#contribuir)
...
- Ejecuta un agente de IA con herramientas, parches y checkpoints
...
- Sin SSH para git — solo HTTPS y tokens
...
Pestañas, vista dividida, vista de diff, visores de imagen/PDF/SQLite. Los búferes sin guardar sobreviven a un reinicio.
...
|---|---|---|
| **Node** | JavaScriptCore + una capa Node escrita a mano: `fs`, `path`, `events`, `buffer`, `stream`, `timers`, `fetch`, `http`, `require` | No es V8. Sin addons nativos. |
| **npm** | Registro real, resolución semver real, `package-lock.json` v3 real, tarballs reales, `.bin` real | Los paquetes con binarios nativos caen a `esm.sh` |
| **Bundler** | `esbuild-wasm` corriendo dentro de JavaScriptCore | Transformación y dev server; `build` solo para Vite |
...
Dev servers que funcionan hoy: **Vite** (`dev`, `build`, `preview`), **Astro** (`dev`), **Next** (`dev`),
**Nest** (vía `node`).
...
826 cadenas viven en un único String Catalog. `make i18n` lo compara contra el código y
...
| `make unit` | Pruebas unitarias de los 15 paquetes (223 pruebas) |
...
| `OdeteNpm` | Registro, semver, `package-lock` v3, tarballs, `.bin`, fallback a `esm.sh` |
...
- Next se renderiza en el servidor, hidrata componentes `'use client'` y ejecuta middleware,
  `next/font` y Server Actions. Falta: `useActionState` muestra su estado inicial hasta que
  la isla hidrata (`renderToString` no recibe el postback), la subida de archivos por una
  acción de formulario, y `'use server'` escrito dentro del componente en vez de al principio
  del archivo
...
`make check` es la puerta: formato, lint, catálogo de traducciones, build, 223 pruebas. El CI corre lo mismo
en cada push.
