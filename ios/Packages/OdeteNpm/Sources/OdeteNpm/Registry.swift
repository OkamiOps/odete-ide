...
public struct Packument: Sendable, Hashable {
    ...
    public static func parse(_ data: Data) throws -> Packument {
        guard let j = try JSONSerialization.jsonObject(with: data) as? [String: Any], let name = j["name"] as? String else { throw NpmError.registry("packument inválido") }
        var out: [Version: PackumentVersion] = [:]
        for (k, v) in (j["versions"] as? [String: Any]) ?? [:] {
            guard let ver = Version(k), let m = v as? [String: Any], let dist = m["dist"] as? [String: Any], let tar = dist["tarball"] as? String else { continue }
            func deps(_ key: String) -> [String: String] { (m[key] as? [String: String]) ?? [:] }
            var bin: [String: String] = [:]
            if let b = m["bin"] as? [String: String] { bin = b } else if let b = m["bin"] as? String {
                let short = name.split(separator: "/").last.map(String.init) ?? name
                bin = [short: b]
            }
            out[ver] = PackumentVersion(version: ver, dependencies: deps("dependencies"), optionalDependencies: deps("optionalDependencies"), peerDependencies: deps("peerDependencies"), bin: bin, tarball: tar, integrity: dist["integrity"] as? String, os: (m["os"] as? [String]) ?? [], cpu: (m["cpu"] as? [String]) ?? [], hasInstallScript: (m["hasInstallScript"] as? Bool) ?? false, deprecated: m["deprecated"] as? String)
        }
    ...
    public func pick(_ spec: String) -> PackumentVersion? {
        ...
        if let tagged = distTags[s.isEmpty ? "latest" : s], let v = Version(tagged), let pv = versions[v] { return pv }
        let range = SemverRange(s)
        if range.isAny, let latest = distTags["latest"], let v = Version(latest), let pv = versions[v] { return pv }
...
public struct HTTPRegistry: RegistryClient {
    ...
    public static func defaultCache() -> URL {
        FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0].appending(path: "odete-npm", directoryHint: .isDirectory)
    }

    public init(base: URL = URL(string: "https://registry.npmjs.org")!, cacheDir: URL = HTTPRegistry.defaultCache(), session: URLSession = .shared) {
    ...
    public func tarball(_ url: String, integrity: String?) async throws -> Data {
        let key = (integrity ?? url).replacingOccurrences(of: "/", with: "_").replacingOccurrences(of: "+", with: "-").replacingOccurrences(of: "=", with: "")
        ...
        guard (200 ..< 300).contains((resp as? HTTPURLResponse)?.statusCode ?? 0) else { throw NpmError.tarball("HTTP ao baixar \(url)") }
