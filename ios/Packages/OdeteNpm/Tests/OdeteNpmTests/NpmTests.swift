...
@Suite struct SemverTests {
    @Test func versionsCompare() {
        #expect(Version("1.2.3")! < Version("1.10.0")!)
        #expect(Version("1.0.0-alpha")! < Version("1.0.0")!)
        #expect(Version("1.0.0-alpha.1")! < Version("1.0.0-alpha.2")!)
        #expect(Version("v2.0.0")?.description == "2.0.0")
        #expect(Version("1.2") == nil)
    }

    @Test(arguments: [
        ("^1.2.3", "1.9.9", true), ("^1.2.3", "2.0.0", false), ("^0.2.3", "0.2.9", true), ("^0.2.3", "0.3.0", false), ("^0.0.3", "0.0.4", false),
        ("~1.2.3", "1.2.9", true), ("~1.2.3", "1.3.0", false), ("~1", "1.9.0", true), ("1.x", "1.5.0", true), ("1.x", "2.0.0", false),
        (">=1.0.0 <2.0.0", "1.5.0", true), (">=1.0.0 <2.0.0", "2.0.0", false), ("1.2.3 - 2.3.4", "2.0.0", true), ("1.2 - 2.3", "2.3.9", true),
        ("*", "9.9.9", true), ("", "1.0.0", true), ("1 || 3", "3.2.1", true), ("1 || 3", "2.0.0", false), ("^1.0.0", "2.0.0-beta", false), ("^1.0.0-beta", "1.0.0-beta.2", true), ("=1.2.3", "1.2.3", true),
    ])
    func satisfies(range: String, version: String, expected: Bool) {
        #expect(SemverRange(range).satisfies(Version(version)!) == expected, "\(range) vs \(version)")
    }
    ...
    @Test func specParsing() {
        ...
        #expect(Installer.Spec("@types/node@22").name == "@types/node" && Installer.Spec("@types/node@22").range == "22")
    }
}
...
final class FakeRegistry: RegistryClient, @unchecked Sendable {
    ...
    func add(_ name: String, _ version: String, deps: [String: String] = [:], bin: [String: String] = [:], files: [String: String] = [:], os: [String] = [], install: Bool = false, latest: Bool = true) {
        let url = "fake://\(name)/\(version).tgz"
        var pkgJSON: [String: Any] = ["name": name, "version": version, "dependencies": deps]
        if !bin.isEmpty { pkgJSON["bin"] = bin }
        var entries = [Tar.Entry(path: "package/package.json", data: try! JSONSerialization.data(withJSONObject: pkgJSON), isDir: false, mode: 0o644, link: nil)]
        for (p, c) in files { entries.append(Tar.Entry(path: "package/" + p, data: Data(c.utf8), isDir: false, mode: p.hasSuffix(".js") ? 0o755 : 0o644, link: nil)) }
        tarballs[url] = GzipCodec.compress(Tar.write(entries))!
        let pv = PackumentVersion(version: Version(version)!, dependencies: deps, optionalDependencies: [:], peerDependencies: [:], bin: bin, tarball: url, integrity: nil, os: os, cpu: [], hasInstallScript: install, deprecated: nil)
        var p = packuments[name] ?? Packument(name: name, distTags: [:], versions: [:])
        p.versions[pv.version] = pv
        if latest { p.distTags["latest"] = version }
        packuments[name] = p
    }
}

func project(_ deps: [String: String]) throws -> URL {
    let u = FileManager.default.temporaryDirectory.appending(path: "odete-npm-\(UUID().uuidString)", directoryHint: .isDirectory)
    try FileManager.default.createDirectory(at: u, withIntermediateDirectories: true)
    try JSONSerialization.data(withJSONObject: ["name": "proj", "dependencies": deps]).write(to: u.appending(path: "package.json"))
    return u
}
...
@Suite struct InstallerTests {
    func registry() -> FakeRegistry {
        let r = FakeRegistry()
        r.add("b", "1.0.0", latest: false); r.add("b", "1.2.0"); r.add("b", "2.0.0", files: ["index.js": "module.exports = 'b2';"])
        r.add("a", "1.0.0", deps: ["b": "^1.0.0"], files: ["index.js": "module.exports = require('b');"])
        r.add("c", "3.1.0", deps: ["b": "^2"], bin: ["cee": "cli.js"], files: ["cli.js": "#!/usr/bin/env node\nconsole.log('cee');"])
    ...
    @Test func installHoistsNestsAndWritesLock() async throws {
        ...
        let lock = Lockfile.load(dir.appending(path: "package-lock.json"))!
    ...
    @Test func addUninstallAndReports() async throws {
        ...
        let rep2 = try await Installer(project: try project(["sowin": "1.0.0", "inexistente": "1"]), registry: reg).install()
    ...
    @Test func tarRoundTripAndLongNames() throws {
        ...
        let entries = [Tar.Entry(path: "package/" + long, data: Data("x".utf8), isDir: false, mode: 0o644, link: nil), Tar.Entry(path: "package/dir", data: Data(), isDir: true, mode: 0o755, link: nil)]
        let back = try Tar.read(Tar.write(entries))
        #expect(back.map(\.path) == ["package/" + long, "package/dir"])
        let gz = GzipCodec.compress(Data("olá".utf8))!
