...
enum HostFetch {
    static func install(_ rt: JSRuntime) {
        let h = rt.host
        let fetch: @convention(block) (Int, String, String, [String: String], JSValue?) -> Void = { [unowned rt] id, url, method, headers, bodyValue in
            ...
            if let bodyB64, let d = Data(base64Encoded: bodyB64) { req.httpBody = d }
            rt.beginWork()
            let task = URLSession.shared.dataTask(with: req) { data, resp, error in
                rt.queue.async {
                    ...
                    rt.call("__odete_fetchDone", [id, http?.statusCode ?? 200, hdrs, (data ?? Data()).base64EncodedString(), http?.url?.absoluteString ?? url])
                }
            }
            task.resume()
        }
