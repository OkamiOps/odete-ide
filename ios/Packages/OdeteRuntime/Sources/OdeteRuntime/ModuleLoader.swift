...
enum ModuleLoader {
    ...
    static func install(_ rt: JSRuntime) {
        ...
        let resolve: @convention(block) (String, String) -> Any = { spec, from in
            if let r = resolveModule(spec, from: from) { return r }
        ...
        let load: @convention(block) (String) -> Any = { [unowned rt] path in
            ...
            if needsTransform {
                guard let t = rt.transform else {
                    return ["error": "ESM", "message": "\(path): módulos ES e TypeScript precisam do transformador (esbuild), que chega no marco 3"]
                }
                do { src = try t(src, path) } catch { return ["error": "TRANSFORM", "message": error.localizedDescription] }
            }
            return src
        }
    ...
    static func looksLikeESM(_ src: String, ext: String) -> Bool {
        ...
        return src.range(of: #"^\s*(import\s|export\s)"#, options: [.regularExpression, .anchored]) != nil
            || src.range(of: #"\n\s*(import\s+[\w{*]|export\s+(default|const|function|class|let|var|\{|\*))"#, options: .regularExpression) != nil
    }

    static func resolveModule(_ spec: String, from: String) -> String? {
        let fromDir = (from as NSString).deletingLastPathComponent
        var s = spec
        if s.hasPrefix("node:") { return "node:" + String(s.dropFirst(5)) }
        ...
        while true {
            ...
            if let r = resolvePackage(s, in: nm) { return r }
    ...
    static func resolveFileOrDir(_ base: String) -> String? {
        ...
        if fm.fileExists(atPath: base, isDirectory: &isDir), !isDir.boolValue { return base }
        for e in extensions where fm.fileExists(atPath: base + e) { return base + e }
        if isDir.boolValue {
            ...
            if let d = fm.contents(atPath: pkg), let j = try? JSONSerialization.jsonObject(with: d) as? [String: Any] {
                if let main = j["main"] as? String, let r = resolveFileOrDir((base as NSString).appendingPathComponent(main)) { return r }
            }
    ...
    static func resolvePackage(_ spec: String, in nodeModules: String) -> String? {
        ...
        let j = FileManager.default.contents(atPath: pkgJSON).flatMap { try? JSONSerialization.jsonObject(with: $0) as? [String: Any] } ?? [:]
        if let exports = j["exports"] {
            let key = sub.isEmpty ? "." : "./" + sub
            if let target = resolveExports(exports, key: key), let r = resolveFileOrDir((pkgDir as NSString).appendingPathComponent(target)) { return r }
        }
        if !sub.isEmpty { return resolveFileOrDir((pkgDir as NSString).appendingPathComponent(sub)) }
        for k in ["main", "module"] {
            if let m = j[k] as? String, let r = resolveFileOrDir((pkgDir as NSString).appendingPathComponent(m)) { return r }
        }
    ...
    static func resolveExports(_ exports: Any, key: String) -> String? {
        if let s = exports as? String { return key == "." ? s : nil }
        ...
        if let v = map[key] { return pickCondition(v) }
        for (pattern, v) in map where pattern.contains("*") {
            let parts = pattern.split(separator: "*", omittingEmptySubsequences: false).map(String.init)
            guard parts.count == 2, key.hasPrefix(parts[0]), key.hasSuffix(parts[1]), key.count >= pattern.count - 1 else { continue }
            let star = String(key.dropFirst(parts[0].count).dropLast(parts[1].count))
            if let t = pickCondition(v) { return t.replacingOccurrences(of: "*", with: star) }
        }
        return nil
    }
    ...
    static func pickCondition(_ v: Any) -> String? {
        ...
        if let arr = v as? [Any] { for x in arr { if let r = pickCondition(x) { return r } }; return nil }
        guard let map = v as? [String: Any] else { return nil }
        for cond in ["node", "require", "import", "module", "default"] {
            if let x = map[cond], let r = pickCondition(x) { return r }
        }
        return nil
    }
}
